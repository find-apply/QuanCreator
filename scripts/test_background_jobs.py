...
BASE_URL = "https://randolph-uncongratulating-unprofessorially.ngrok-free.dev/api"
    ...
    if result:
        ...
        if status.get('completed', 0) > 0:
            ...
            try:
                ...
                if not results:
                    print("Results not found in embedded session. Attempting to fetch session...")
                    session_id = queue_item.get('session_id')
                    if session_id:
                        try:
                            session_response = requests.get(f"{BASE_URL}/v1/sessions/{session_id}")
                            if session_response.status_code == 200:
                                session = session_response.json()
                                results = session.get('results', {})
                            else:
                                print(f"Warning: Could not fetch session {session_id}. Status: {session_response.status_code}")
                        except Exception as e:
                             print(f"Warning: Error fetching session: {e}")
